...
public class ImportPaymentReceiptModel
{
...
public class ImportPaymentReceiptDocument : IDocument
{
    private readonly ImportPaymentReceiptModel _model;
    private readonly Company _company;

    public ImportPaymentReceiptDocument(ImportPaymentReceiptModel model, Company company)
    {
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Column(column =>
        {
            ...
            column.Item().PaddingTop(8).AlignCenter().Text("سداد مورد استيراد").FontSize(18).SemiBold().FontColor(Colors.Black);
    ...
    private void ComposeContent(IContainer container)
    {
        container.PaddingVertical(5).Column(col =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(_model.OmlaName))
                fields.Add(new("العملة", _model.OmlaName));
            if (!string.IsNullOrWhiteSpace(_model.InvNo))
                fields.Add(new("رقم الفاتورة", _model.InvNo));
    ...
    private void ComposeTotals(IContainer container)
    {
        ...
        {
            new("المبلغ بالعملة",         _model.Amount.ToString("N2")),
            new("سعر الصرف",              _model.OmlaRate.ToString("N4")),
            new("المبلغ بالجنيه",         (_model.Amount * (double)_model.OmlaRate).ToString("N2")),
