    ...
    public async Task<double> GetCashOldBalanceAsync(int cashId, DateTime toDate)
    {
        ...
        const string sql = @"
            DECLARE @Bal FLOAT = 
                ...
                + ISNULL((SELECT SUM(PayMoneyTo) FROM Cash_Transfer WHERE CashTo = @CashId AND PayDate < @ToDate), 0)
    ...
    public async Task RecalcBalanceForCashAsync(int cashId)
    {
        ...
        const string sql = @"
            UPDATE Cash SET Bal = 
                ...
                + ISNULL((SELECT SUM(PayMoneyTo) FROM Cash_Transfer WHERE CashTo = @CashId), 0)
